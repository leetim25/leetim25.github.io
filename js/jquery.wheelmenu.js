...
!function($){
  ...
  $.fn.fadeInIcon = function (el, button, width, height, angle, step, radius, settings) {
    ...
    this.each(function(index) {
      ...
      var x = Math.round(width/2 + radius * Math.cos(angle) - $(this).find("img").height()/2-60),
          y = Math.round(height/2 + radius * Math.sin(angle) - $(this).find("img").height()/2);
      $(this).css({
          position: 'absolute',
          left: x + 'px',
          top: y + 'px',
  ...
  $.fn.fadeOutIcon = function (el, button) {
	...
	function predefineSpeed(settings) {
      ...
      switch (settings.animationSpeed) { 
        case 'slow':
          settings.animationSpeed = [75,7000]
          break;
        case 'medium':
